        ...
        for {
            select {
                case augmentedSay := <-chatRoom.incoming: {
                 ...
                 } else if len(say) >= 4 && string(say[0:4]) == "exit" {
                    	senderClient := chatRoom.clients[senderId - 1]
                    	 fmt.Println("Closing connect ...")
                    	senderClient.connection.Close() 
...
func main() {
    ...
    if  "-c" == os.Args[1] {
    	...
    	if err != nil {
    		fmt.Println("Unable to start the client")
    	}else{    
      ...
      var serverport string = os.Args[1]
